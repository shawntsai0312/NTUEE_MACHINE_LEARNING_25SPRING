...
def sce(
    ...
    '''
    Merge the task tensors using Select-Combine-Extract (SCE) method.
    Reference: https://arxiv.org/abs/2408.07990

    Args:
        task_tensors (List[torch.Tensor]): The task tensors to merge.
        base_tensor (Optional[torch.Tensor]): A tensor to add after merging (e.g., base model delta).
        density (float): Fraction of elements to retain (top-k variance). Should be in [0, 1].
        majority_sign_method (str): How to compute sign mask. Options: 'total', 'frequency'.
    ...
    stacked = torch.stack(task_tensors, dim=0)  # shape: [T, ...]
    device = stacked.device

    # Sparsity mask
    if density < 1.0:
        var_mask = sce_mask(stacked, density=density, mask_dtype=stacked.dtype).to(device)
    else:
        var_mask = torch.ones_like(stacked[0], dtype=stacked.dtype)  # note: shape [D]

    # Apply mask before sign and weight computation
    masked_stack = stacked * var_mask  # shape [T, ...]

    # Sign filtering on masked values
    sign_sum = torch.sign(masked_stack).sum(dim=0)
    sign_mask = (sign_sum.abs() >= 1).to(dtype=stacked.dtype)

    # Compute weights on masked values
    weights = sce_weight(masked_stack)  # shape: [T]

    # Combine with sign mask
    weights = weights.view([-1] + [1] * (stacked.dim() - 1)) * sign_mask
    weighted = masked_stack * weights
    ...
    return merged + base_tensor if base_tensor is not None else merged
