...
class LoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def _check_new_adapter_config(self, config: LoraConfig) -> None:
        ...
        #     )
        if any(c.bias != "none" for c in self.peft_config.values()) and (config.bias != "none"):
            raise ValueError(
                f"{self.__class__.__name__} supports only 1 adapter with bias. When using multiple adapters, "
                "set bias to 'none' for all adapters."
            )
    ...
    def _generalized_task_arithmetic_weighted_adapter(
        ...
        for i, task_tensors in enumerate(lora_deltas):
            ...
            # print(f"debug: {combination_type}")
            if combination_type == "linear":
                ...
                base_tensor = None
                if hasattr(target, "base_lora_A") and i == 0:
                    base_tensor = target.base_lora_A
                elif hasattr(target, "base_lora_B") and i == 1:
                    base_tensor = target.base_lora_B
                lora_deltas[i] = sce(
